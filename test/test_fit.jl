@testitem "OLS coefficients - simple" tags = [:ols, :smoke] begin
    ...
    m = @formula Sales ~ Price
    x = Regress.ols(df, m, weights = :Pop)
...
@testitem "IV coefficients - multiple endogenous" tags = [:iv, :tsls] begin
    ...
    # iv + weight
    m = @formula Sales ~ (Price ~ Pimin)
    x = Regress.iv(Regress.TSLS(), df, m, weights = :Pop)
...
@testitem "Programmatic formula construction" tags = [:ols, :formula] begin
    ...
    Regress.ols(df, term(:Sales) ~ term(:NDI) + fe(:State) + fe(:Year))
...
@testitem "IV collinearity" tags = [:iv, :tsls] begin
    ...
    xNDI = Regress.iv(Regress.TSLS(), df, m2)
...
@testitem "OLS standard errors - cluster" tags = [:ols, :vcov, :cluster] begin
    ...
    m = @formula Sales ~ Price + fe(State)
    x = Regress.ols(df, m, save_cluster = :Year)
...
@testitem "OLS standard errors - multiway" tags = [:ols, :vcov, :cluster] begin
    ...
    m = @formula Sales ~ Price
    x = Regress.ols(df, m, save_cluster = [:State, :Year])
...
@testitem "IV standard errors - cluster" tags = [:iv, :vcov, :cluster] begin
    ...
    m = @formula Sales ~ CPI + (Price ~ Pimin)
    x = Regress.iv(Regress.TSLS(), df, m, save_cluster = :State)
    ...
    x = Regress.iv(Regress.TSLS(), df, m, save_cluster = :Year)
...
@testitem "subset option" tags = [:ols, :iv] begin
    ...
    x0 = Regress.ols(df[df.State .<= 30, :], m)

    m = @formula Sales ~ Price + StateC
    xPrice = Regress.ols(df, m, subset = df.State .<= 30)
    ...
    x = Regress.ols(df, m, weights = :Price_missing)
    ...
    m = @formula Sales ~ Price + fe(State)
    x3 = Regress.ols(df, m, subset = df.State .<= 30)
    ...
    m = @formula Sales ~ Price + fe(State)
    x5 = Regress.ols(df, m, subset = df.State .>= 30)

    #Error reported by Erik - cluster vcov with subset
    m = @formula Sales ~ Pimin + CPI
    x = Regress.ols(df, m, save_cluster = :State, subset = df.State .>= 30)
...
@testitem "loglikelihood" tags = [:ols, :statistics] begin
    using Regress, CategoricalArrays, CSV, DataFrames
    using Regress: fe, nullloglikelihood_within
...
@testitem "singletons" tags = [:ols, :fe] begin
    ...
    m = @formula Sales ~ Price + fe(pn)
    x = Regress.ols(df, m, drop_singletons = false)
    @test x.nobs == 1380
end
...
@testitem "unbalanced panel" tags = [:ols, :fe] begin
    ...
    m = @formula Wage ~ Emp + fe(Firm)
    x = Regress.ols(df, m, weights = :Output)
...
@testitem "GPU acceleration" tags = [:ols, :fe, :gpu] begin
    ...
    for method in methods_vec
        ...
        local x = Regress.ols(df, m, method = method, double_precision = false)
        ...
        local m = @formula Wage ~ Emp + fe(Firm)
        local x = Regress.ols(
            df, m, weights = :Output, method = method, double_precision = false)
