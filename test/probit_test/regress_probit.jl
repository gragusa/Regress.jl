...
const PROBIT_FORMULA = @formula(visit_dummy ~
                                age + hhninc + hhkids + educ + married + fe(id) + fe(year))
...
if abspath(PROGRAM_FILE) == @__FILE__
    output_csv = length(ARGS) >= 1 ? ARGS[1] :
                 joinpath(@__DIR__, "regress_probit_results.csv")
